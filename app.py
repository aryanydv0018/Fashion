...
if upload_file is not None:
    ...
    st.write("Input Image Features Shape:", input_img_features.shape)
    distance, indices = neighbors.kneighbors([input_img_features])
    st.write("Distances:", distance)
    ...
    for i, idx in enumerate(indices[0][1:6], 1):  # Show 5 recommendations (skip input image)
        recommended_path = os.path.join(BASE_DIR, filenames[idx])
        if os.path.exists(recommended_path):
            if i == 1:
                with col1:
                    st.image(recommended_path, caption=f"Rec {i}")
            elif i == 2:
                with col2:
                    st.image(recommended_path, caption=f"Rec {i}")
            elif i == 3:
